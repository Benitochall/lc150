...
def dij_between(graph, node1,node2):
    ...
    while len(discoverdSet) < graph.size and node2 not in discoverdSet:
        ...
        for key, value in distSet.items():
            if value < mini and key not in discoverdSet:
                mini = value
                closest_node = key
        ...
        if closest_node:
            ...
            current_dist = distSet[closest_node]
           
            for item in ajacents:
                if item[0] in discoverdSet:
                    continue
                if distSet[item[0]] > item[1] + current_dist:
                    distSet[item[0]] = item[1] + current_dist
    return distSet[node2]
